...
export const Pagination = ({ total, offset, limit, onChange, noun = "agents", testId = "pagination" }: {
  total: number; offset: number; limit: number; onChange: (offset: number) => void; noun?: string; testId?: string;
}) => {
  ...
  return <div className="pager" data-testid={testId}>
    ...
    <span data-testid={`${testId}-label`}>
      ...
      <em>{total.toLocaleString()} {noun}</em>
    </span>
