...
export default function AgentPage() {
  ...
  return <div className="page-wrap agent-detail-page">
    ...
    <div className="detail-layout"><div className="evidence-column">
      ...
      <section className="plain-section"><div className="section-title"><h2 data-testid="identity-heading">Identity & claims</h2><span className="source-pill"><ShieldCheck size={13}/> Onchain source</span></div><div className="identity-list"><div><span>Network</span><strong data-testid="identity-network">BSC Testnet · Chain 97</strong></div><div><span>ERC-8004 agent ID</span><strong data-testid="identity-agent-id">#{agent.identity.agent_id}</strong></div><div><span>Metadata</span><strong data-testid="identity-metadata">{agent.identity.metadata_verified ? "Registration checked" : "Claim not verified"}</strong></div><div><span>Endpoint domain</span><strong data-testid="identity-endpoint">Pending live endpoint</strong></div></div><a data-testid="registry-reference-link" href={`https://testnet.bscscan.com/address/${agent.identity.registry}`} target="_blank" rel="noreferrer">Registry reference <ExternalLink size={13}/></a></section>
    ...
    </div><aside className="task-panel" data-testid="task-composer"><div className="task-panel-heading"><span>Hire agent</span><strong>${agent.price_usd.toFixed(2)}</strong></div><label htmlFor="objective">Research objective</label><Textarea id="objective" data-testid="task-objective-input" value={objective} onChange={e=>setObjective(e.target.value)} rows={6}/><label htmlFor="constraints">Constraints</label><Textarea id="constraints" data-testid="task-constraints-input" value={constraints} onChange={e=>setConstraints(e.target.value)} rows={4}/><div className="transaction-preview"><h3>Transaction preview</h3><div><span>Network</span><strong data-testid="preview-network">BSC Testnet</strong></div><div><span>Maximum cost</span><strong data-testid="preview-cost">${agent.price_usd.toFixed(2)}</strong></div><div><span>Recipient</span><strong data-testid="preview-recipient">Assigned by B402 quote</strong></div><div><span>Permissions</span><strong data-testid="preview-permissions">Payment only · no approval</strong></div></div>
      {!address ? <WalletButton /> : chainId !== 97 ? <div className="inline-warning" data-testid="wrong-network-warning"><AlertTriangle size={15}/> Switch to BSC Testnet</div> : null}
